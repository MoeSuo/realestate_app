...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resetLink = `${baseURL}/user/setNewPassword?token=${resetToken}&email=${email}`;    const msg = {
      to: email,
...
<div style="font-family: inherit; text-align: inherit">To regain access to your lush green world in the Garden App, simply Click on the "Reset Password" link below:&nbsp;</div><div></div></div></td>
    ...
    res.status(500).json({ error: 'An error occurred while sending the reset password email' });
  } finally {
    prisma.$disconnect();
  }
}